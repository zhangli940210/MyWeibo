...
@implementation LZEmotionTextView
...
- (void)insertEmotion:(LZEmotion *)emotion
{
    if (emotion.code) {
        ...
        // 加载图片
        LZEmotionAttachment *attch = [[LZEmotionAttachment alloc] init];
        
        // 传递模型
        attch.emotion = emotion;
        
        // 设置图片的尺寸
        CGFloat attcLZH = self.font.lineHeight;
        attch.bounds = CGRectMake(0, -4, attcLZH, attcLZH);
...
- (NSString *)fullText
{
    ...
    [self.attributedText enumerateAttributesInRange:NSMakeRange(0, self.attributedText.length) options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
        // 如果是图片表情
        LZEmotionAttachment *attch = attrs[@"NSAttachment"];
        if (attch) { // 图片
